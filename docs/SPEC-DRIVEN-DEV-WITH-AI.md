...
**The process:** Idea → Interactive grilling → Formal spec → Architecture → Code → Well-Architected review → Hardening → Tests green.
...
That last line is the key. Instead of letting the AI run off and generate code, we explicitly requested a **structured interrogation**. This sets the tone: the AI is not a code monkey — it's a co-architect who must understand the problem before touching a keyboard.

### Lesson: The "Ask Me" Prompt

Starting with "ask me" instead of "build me" fundamentally changes the AI's behavior. It shifts from solution mode to discovery mode, which is where the real value lies for non-trivial systems.
...
| 2 | Volume and size? | Small — handful of files, under 50 MB | Justified in-memory processing |
...
| 5 | One archive or per-file? | One .zip, one .tar (all files) + individual .gz | Defined the strategy pattern |
| 6 | Output destination? | Same bucket, different prefix | Kept infrastructure simple |
| 7 | Schedule frequency? | Daily | Justified reserved concurrency = 1 |
| 8 | Error handling? | Skip failed, compress the rest, report | Defined partial failure model |
| 9 | Runtime mode? | GraalVM native image | Drove build tooling choices |
| 10 | Local testing tool? | Floci (not LocalStack) | AI didn't know Floci — had to look it up |
...
When we said "Floci," the AI assumed we meant LocalStack or had a typo. We insisted. The AI searched, found [floci.io](https://floci.io), and adopted it — an MIT-licensed, Docker-based AWS emulator with no auth token required. This moment illustrates an important dynamic: **the human brings domain knowledge the AI doesn't have.** The AI's job is to integrate that knowledge, not override it.

---

## Phase 3: The Formal Spec (15 minutes)

With all 12 decisions locked, the AI produced a formal **Software Design Document (SPEC.md)** with:

- **7 Functional Requirements** (FR-1 through FR-7) with traceable IDs
- **4 Non-Functional Requirements** (performance, runtime, config, observability)
- **6 Edge Cases** (empty prefix, all downloads fail, single file, special characters, etc.)
- **5 Acceptance Criteria** in Given/When/Then format
- **A test plan** mapping unit tests to strategy classes and integration tests to acceptance criteria
...
2. **Models & Config** — Java records (`FileEntry`, `CompressionResult`), `CompressorConfig` interface
3. **Compression Strategies** — Three classes implementing a `CompressionStrategy` interface + 14 unit tests
4. **S3 Service** — List/download/upload with error handling
5. **Orchestrator & Handler** — Wiring, summary logging, Lambda entry point
6. **Integration Tests** — 3 full-flow tests against Floci
7. **Terraform** — S3, Lambda, IAM, EventBridge, CloudWatch
...
|----------|-------------|
| `FileStorageService` interface over `S3FileService` | **YAGNI** — there's only ever one storage backend. An interface adds indirection with zero benefit. |
| `AbstractArchiveStrategy` base class | **Accidental similarity** — Zip, Tar, and Gzip strategies look similar but differ enough (streams, multi-output) that a shared base class would add complexity without reducing bugs. |
...
| Proposal | Why Kept |
|----------|---------|
| `CompressionResult` builder | Justified by growing field count (7+ fields including new `strategyFailures`) — factory methods become unwieldy. Builder auto-derives status from content. |
| `TestDecompressor` utility | Clear DRY win — identical decompress logic duplicated across 4 test classes. |

### The Principle

**DRY is about knowledge duplication, not code duplication.** Three strategies that happen to write bytes to a stream aren't duplicating knowledge — they're independently implementing different compression algorithms. Forcing them into a base class conflates accidental similarity with genuine duplication.
...
Here's the reusable plan of execution, distilled from this experience:
...
1. Walk through AWS Well-Architected Framework (or your organization's review framework)
2. Go pillar by pillar — don't skip any
3. Catalogue every gap with a specific fix
4. Track items in a numbered gap list
...
3. Keep changes that solve real problems (builder for 7-field records, shared test utilities)
4. Apply modern language features only where they genuinely simplify

### Stage 6: Implementation of Gaps
1. Batch related changes (spec updates, Java code, config, Terraform, tests)
...
These are the non-obvious decisions that shaped the project. Keep these in mind for future projects:
...
*This guide was produced as a companion to the [S3 File Compressor Lambda](../SPEC.md) project specification.*
